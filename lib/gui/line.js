...
class Line extends Box{
  ...
  _drawLine() {
    ...
    if (!this.data.fitToScreen && (this.data.y.length - this.canvasSize.width + this.chartLeftPadding) > 0) {
      for(var k = this.data.y.length - this.canvasSize.width + this.chartLeftPadding + this.scrollValue; k < this.data.y.length; k++) {
  ...
  _drawXLabels () {
    ...
    for(var i = 0; i < this.data.x.length; i += showNthLabel) {
      if((this._getXPixel(i, this.data.x.length, this.chartLeftPadding) + (this.data.x[i].length * 2)) <= this.canvasSize.width) {
        this.ctx.fillText(this.data.x[i], this._getXPixel(i, this.data.x.length, this.chartLeftPadding), this.canvasSize.height - CHART_TOP_PADDING + 5);
      }
    }
  }
