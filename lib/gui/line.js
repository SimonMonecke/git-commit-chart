...
class Line extends Box{
  ...
  _getXPixel(val) {
      return ((this.canvasSize.width - this.chartLeftPadding) / this.data.y.length) * val + this.chartLeftPadding;
  }

  _getYPixel(val) {
      return this.canvasSize.height - CHART_TOP_PADDING - (((this.canvasSize.height - CHART_TOP_PADDING) / this.maxY) * val);
  }

  _getYValue(pixel, maxY) {
      return (this.canvasSize.height - CHART_TOP_PADDING - pixel) / ((this.canvasSize.height - CHART_TOP_PADDING) / this.maxY);
  }

  _drawLine() {
    this.ctx.strokeStyle = LINE_COLOR;
    const chartWidth = this.canvasSize.width - this.chartLeftPadding;
    const countOfValues = this.data.y.length;

    this.ctx.moveTo(0, 0)
    this.ctx.beginPath();

    if (!this.data.fitToScreen && countOfValues > chartWidth) {
      for(var k = 0; k < chartWidth; k++) {
          const index = k + countOfValues - chartWidth + this.scrollValue;
          this.ctx.lineTo(k + this.chartLeftPadding, this._getYPixel(this.data.y[index]));
      }
      this.ctx.fillText(`${countOfValues - chartWidth + this.scrollValue}-${countOfValues + this.scrollValue}/${countOfValues}`, this.chartLeftPadding, this.canvasSize.height - CHART_TOP_PADDING + 5);
    } else {
  ...
  _drawXLabels () {
    ...
    for(var i = 0; i < this.data.x.length; i += showNthLabel) {
      if((this._getXPixel(i) + (this.data.x[i].length * 2)) <= this.canvasSize.width) {
        this.ctx.fillText(this.data.x[i], this._getXPixel(i), this.canvasSize.height - CHART_TOP_PADDING + 10);
      }
    }
  }
