...
function getMaxXLabelPadding(longestLabel) {
  return formatYLabel(longestLabel).length;
}
...
Line.prototype.setData = function(data) {
    ...
    function getXPixel(val) {
        return ((self.canvasSize.width - xPadding) / data.x.length) * val + (xPadding * 1.0);
    }
    ...
    function drawLine(values) {
      ...
      c.moveTo(0, 0)
      c.beginPath();
      c.lineTo(getXPixel(0) + 2, getYPixel(values[0]));

      for(var k = 1; k < values.length; k++) {
          c.lineTo(getXPixel(k) + 2, getYPixel(values[k]));
      }
      c.stroke();
    }
    ...
    for(var i = 0; i < maxY; i += yLabelIncrement) {
        c.fillText(formatYLabel(i), xPadding - maxLabelPadding - 5, getYPixel(i));
        c.strokeStyle = 'green';
        c.moveTo(xPadding, getYPixel(i));
        c.beginPath();
        c.lineTo(xPadding - 1, getYPixel(i));
        c.lineTo(xPadding , getYPixel(i));
        c.stroke();
    }
