...
Line.prototype.setData = function(data) {
    ...
    function drawLine(values) {
      ...
      if (!data.fitToScreen && (values.length - self.canvasSize.width + xPadding) > 0) {
        for(var k = values.length - self.canvasSize.width + xPadding + data.scrollLeft; k < values.length; k++) {
            c.lineTo(k - values.length + self.canvasSize.width - data.scrollLeft, getYPixel(values[k]));
        }
      } else {
